...
Post engagement (#23) adds twelve command-line verbs' worth of surface — `post boost`/`unboost`,
`favorite`/`unfavorite`, `pin`/`unpin`, and `post show` — over what is, underneath, three switches and a read. Four
decisions keep it that size.
...
the marks are sent, and a refusal is reported in the instance's own words through ADR-0006's one handler. Asking for a
mark a post already carries is not an error either: the post ends up as asked, which is what lets a script say what it
wants without first asking what is there.

**A post is written down in one place, whichever command printed it.** `post show` prints what `timeline home` prints
for the same post, because both call `PostReport.Write` — the rendering moved out of `TimelineReport`, which now only
decides that posts come one after another with a blank line between them. ADR-0008 gave the same argument for
...
`post show` prints no web address, because a timeline's posts do not and there is one rendering. Anything wanting the
address has `--json`, which carries it. If reading one post turns out to want more on screen than a timeline's entry
does, the answer is a second method on `PostReport` that a timeline does not call — not a second idea of what a post
looks like.
