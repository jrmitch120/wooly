...
/// <summary>
///     The one crossing between Mastodon's <c>tag</c> and this project's <see cref="Hashtag" />, named for the wire
///     type it crosses as <see cref="Posts.PostWire" /> and <see cref="Accounts.AccountWire" /> are, and for the same
///     reason: one mapping, so a hashtag looks the same however it arrived.
/// </summary>
/// <remarks>
///     Two things here are called a hashtag, and they are a value and a rule: <see cref="Hashtag" /> is a tag a search
///     turned up, and <c>Timelines.Hashtag</c> — aliased above — is the rule about what a tag may be, which the tag
///     timeline is read by. The alias is what says which is meant at each use.
/// </remarks>
internal static class HashtagWire
{
